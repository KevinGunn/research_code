...
select subject_id, icustay_id, itemid, valuenum, charttime as he_time,
    ...
    FROM chartevents_adult WHERE itemid = 456 or itemid = 52 or itemid = 6702  --in ( 456, 52, 6702 )
),
...
--select count(distinct icustay_id) from hypoentries2;
...
hypoexits2 as(
    ...
    FROM hypoexits h WHERE valuenum is not null and valuenum > (select exit_hypo_threshold from dual) and valuenum <= 180
                                       and windowend > (select exit_hypo_threshold from dual) and windowend <= 180 and icustay_id is not null
    								   and itemid = windowend_itemid
),
...
select itemid, count(subject_id) from he_set1 group by itemid;


select subject_id, he_onset from he_set2 group by
, he_set_overlap


-- intime is admittime to icu for each patient.
select dbsource, hadm_id, HE_set1.*, intime, los
	into HE_Cohort
    from he_set1
	INNER JOIN icustays
...
WITH icustay_CMO AS (
    select h.icustay_id, chartevents_adult.itemid, chartevents_adult.value, chartevents_adult.charttime as CMO_time, h.he_length, h.he_onset, h.he_offset 
         ...
         inner join he_cohort h on chartevents_adult.icustay_id = h.icustay_id
          where  value in ('Comfort measures only') and
                        (h.he_onset - chartevents_adult.charttime ) between '-24:00:00' and '00:00:00' or
                   value in ('Comfort measures only') and
                        (h.he_offset - chartevents_adult.charttime ) between '00:00:00' and '24:00:00'
    		)
...
select itemid, count(subject_id) from  hypo_cohort_final_cv group by itemid;
...
select subject_id, icustay_id, itemid, valuenum, charttime as he_time,
    ...
    last_value(valuenum) over (partition by subject_id, icustay_id order by charttime ROWS BETWEEN CURRENT ROW and 1 following ) as windowend,
    last_value(itemid) over (partition by subject_id, icustay_id order by charttime ROWS BETWEEN CURRENT ROW and 1 following ) as windowend_itemid
    FROM chartevents_adult WHERE itemid in ( 220181, 220052, 225312 )
),
--select * from hypoentries limit 50;
...
WITH icustay_CMO AS (
    select h.icustay_id, chartevents_adult.itemid, chartevents_adult.value, chartevents_adult.charttime as CMO_time, h.he_length, h.he_onset, h.he_offset 
         from chartevents_adult
         inner join he_cohort2 h on chartevents_adult.icustay_id = h.icustay_id
...
select * into hypo_cohort_final_mv from he_cohort2
where icustay_id not in (select distinct icustay_id from icustay_CMO);
