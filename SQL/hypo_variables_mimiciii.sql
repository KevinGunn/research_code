...
mean MAP in the 3 h period immediately prior to the HE onset (measure of
haemodynamic status before the HE), 
...
select * into HYPO_DS_ELIX_3hr_covs7 from HYPO_DS_ELIX_3hr_covs6 where pre_creat is not null;

--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

/* Final Cohort */

select * into HYPO_FINAL_DS from HYPO_DS_ELIX_3hr_covs7 order by subject_id;
...
select count(*) from HYPO_DS_ELIX_3hr_covs7
where dbsource != 'metavision'; -- 2,223 subjects from carevue.


select count(*) from HYPO_DS_ELIX_3hr_covs7
where dbsource = 'metavision'; -- 1,278 subjects from metavision.

select count(*) from HYPO_DS_ELIX_3hr_covs7; -- 3,381 subjects in entire database.

/* Breakdown by treatment */
-- carevue
select count(*) from HYPO_DS_ELIX_3hr_covs7
where dbsource != 'metavision' and iv_fluid_ind = 1 and vp_ind = 0 and rise_creat is not null; -- 212 subjects from carevue with IV Fluid administration.
...
/* Full Data set */
select count(*) from HYPO_DS_ELIX_3hr_covs7
where iv_fluid_ind=1 and vp_ind=0 and rise_creat is not null; -- 464 with IV fluid administration.

select count(*) from HYPO_DS_ELIX_3hr_covs7
where iv_fluid_ind=0 and vp_ind=1 and rise_creat is not null; -- 483 with vasoactive therapy.
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
select * from HYPO_DS_ELIX_3hr_covs7 limit 5;
/* Examine Full data set */

select count(*) from HYPO_DS_ELIX_3hr_covs7 where gender='M'; --1,881
select count(*) from HYPO_DS_ELIX_3hr_covs7 where HOSPITAL_EXPIRE_FLAG=1; --359
select count(*) from HYPO_DS_ELIX_3hr_covs7; --3381

/* Find quartiles for he_length, LOS, AGE */
select k, percentile_disc(k) within group (order by he_length)
from HYPO_DS_ELIX_3hr_covs7, generate_series(0.25, 0.75, 0.25) as k 
group by k;
...
select servicemod, count(subject_id) from HYPO_DS_ELIX_3hr_covs7 group by servicemod;

/* Service breakdown by Services table */
select curr_service, count(subject_id) from HYPO_DS_ELIX_3hr_covs7 group by curr_service;
...
select count(*) from HYPO_DS_ELIX_3hr_covs7 where dbsource !='metavision' and gender='M'; -- 1,194
select count(*) from HYPO_DS_ELIX_3hr_covs7 where dbsource !='metavision' and HOSPITAL_EXPIRE_FLAG=1; -- 226
select count(*) from HYPO_DS_ELIX_3hr_covs7 where dbsource !='metavision'; --2128
...
select servicemod, count(subject_id) from HYPO_DS_ELIX_3hr_covs7 where dbsource!='metavision' group by servicemod;

/* Service breakdown in CareVue Data by Services table */
select curr_service, count(subject_id) from HYPO_DS_ELIX_3hr_covs7 where dbsource!='metavision' group by curr_service;

---------------------------------------------------------------------------------------------------------------------------------------------------------

/* Examine Metavision data set */

select count(*) from HYPO_DS_ELIX_3hr_covs7 where dbsource ='metavision' and gender='M'; -- 687
select count(*) from HYPO_DS_ELIX_3hr_covs7 where dbsource ='metavision' and HOSPITAL_EXPIRE_FLAG=1; -- 133
select count(*) from HYPO_DS_ELIX_3hr_covs7 where dbsource ='metavision'; --1253
...
select * from HYPO_DS_ELIX_3hr_covs7;
