...
/* Number of subjects from each databsae. */
select count(*) from HYPO_DS_ELIX_3hr_covs6
where dbsource = 'carevue'; -- 2,223 subjects from carevue.
...
select count(*) from HYPO_DS_ELIX_3hr_covs6 where gender='M'; --1,957
select count(*) from HYPO_DS_ELIX_3hr_covs6 where HOSPITAL_EXPIRE_FLAG=1; --367
select count(*) from HYPO_DS_ELIX_3hr_covs6; --3513

/* Find quartiles for he_length, LOS, AGE */
select k, percentile_disc(k) within group (order by he_length)
from HYPO_DS_ELIX_3hr_covs6, generate_series(0.25, 0.75, 0.25) as k 
group by k;
...
select servicemod, count(subject_id) from HYPO_DS_ELIX_3hr_covs6 group by servicemod;

/* Service breakdown by Services table */
select curr_service, count(subject_id) from HYPO_DS_ELIX_3hr_covs6 group by curr_service;
...
select count(*) from HYPO_DS_ELIX_3hr_covs6 where dbsource !='metavision' and gender='M'; -- 1,258
select count(*) from HYPO_DS_ELIX_3hr_covs6 where dbsource !='metavision' and HOSPITAL_EXPIRE_FLAG=1; -- 233
select count(*) from HYPO_DS_ELIX_3hr_covs6 where dbsource !='metavision'; --2235
...
select servicemod, count(subject_id) from HYPO_DS_ELIX_3hr_covs6 where dbsource!='metavision' group by servicemod;

/* Service breakdown in CareVue Data by Services table */
select curr_service, count(subject_id) from HYPO_DS_ELIX_3hr_covs6 where dbsource!='metavision' group by curr_service;

---------------------------------------------------------------------------------------------------------------------------------------------------------

/* Examine Metavision data set */

select count(*) from HYPO_DS_ELIX_3hr_covs6 where dbsource ='metavision' and gender='M'; -- 699
select count(*) from HYPO_DS_ELIX_3hr_covs6 where dbsource ='metavision' and HOSPITAL_EXPIRE_FLAG=1; -- 134
select count(*) from HYPO_DS_ELIX_3hr_covs6 where dbsource ='metavision'; --1278
